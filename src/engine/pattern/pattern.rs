...
/// motif with the named operation at its root. Fusing at home is the
/// extra power a variant may carry, answered by [`Pattern::fused`];
/// a homing motif is stored only on forward-only plans, so
/// engine-backward memory contracts stay exact. Dispatch is a plain
/// `match`, the same shape as `Function`.
#[derive(Debug, Clone)]
...
impl Pattern {
    /// Returns the window-GEMM group this motif fuses at home, if any:
    /// the one policy point deciding which variants replace their root
    /// with a payload call in `Plan::forward`. The return type widens
    /// to an enum when a second homing motif lands.
